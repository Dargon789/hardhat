...
describe("HardhatError", () => {
  describe("Type guard", () => {
    it("Should return true for HardhatErrors", () => {
      chai_1.assert.isTrue(
        errors_1.HardhatError.isHardhatError(
          new errors_1.HardhatError(mockErrorDescriptor)
        )
      );
    });
    it("Should return false for everything else", () => {
      ...
      chai_1.assert.isFalse(
        errors_1.HardhatError.isHardhatError(
          new errors_1.NomicLabsHardhatPluginError("asd", "asd")
        )
      );
  ...
  describe("Without parent error", () => {
    ...
    it("should have the right error message", () => {
      ...
      chai_1.assert.equal(
        error.message,
        `HH123: ${mockErrorDescriptor.message}`
      );
    });
    ...
    it("shouldn't have a parent", () => {
      chai_1.assert.isUndefined(
        new errors_1.HardhatError(mockErrorDescriptor).parent
      );
    });
...
describe("Error ranges", () => {
  ...
  it("Should have max > min", () => {
    for (const errorGroup of (0, unsafe_1.unsafeObjectKeys)(
      errors_list_1.ERROR_RANGES
    )) {
      const range = errors_list_1.ERROR_RANGES[errorGroup];
      chai_1.assert.isBelow(
        range.min,
        range.max,
  ...
  it("Shouldn't overlap ranges", () => {
    for (const errorGroup of (0, unsafe_1.unsafeObjectKeys)(
      ...
      const range = errors_list_1.ERROR_RANGES[errorGroup];
      for (const errorGroup2 of (0, unsafe_1.unsafeObjectKeys)(
        errors_list_1.ERROR_RANGES
      )) {
...
describe("Error descriptors", () => {
  it("Should have all errors inside their ranges", () => {
    for (const errorGroup of (0, unsafe_1.unsafeObjectKeys)(
      errors_list_1.ERRORS
    )) {
      const range = errors_list_1.ERROR_RANGES[errorGroup];
      for (const [name, errorDescriptor] of Object.entries(
        errors_list_1.ERRORS[errorGroup]
      )) {
        chai_1.assert.isAtLeast(
          errorDescriptor.number,
          range.min,
          `ERRORS.${errorGroup}.${name}'s number is out of range`
        );
        chai_1.assert.isAtMost(
          errorDescriptor.number,
          range.max - 1,
          `ERRORS.${errorGroup}.${name}'s number is out of range`
        );
  ...
  it("Shouldn't repeat error numbers", () => {
    for (const errorGroup of (0, unsafe_1.unsafeObjectKeys)(
      ...
      for (const [name, errorDescriptor] of Object.entries(
        ...
        for (const [name2, errorDescriptor2] of Object.entries(
          errors_list_1.ERRORS[errorGroup]
        )) {
          if (name !== name2) {
...
describe("HardhatPluginError", () => {
  describe("Type guard", () => {
    it("Should return true for HardhatPluginError", () => {
      chai_1.assert.isTrue(
        errors_1.HardhatPluginError.isHardhatPluginError(
          new errors_1.HardhatPluginError("asd", "asd")
        )
      );
    });
    it("Should return false for everything else", () => {
      chai_1.assert.isFalse(
        errors_1.HardhatPluginError.isHardhatPluginError(new Error())
      );
      chai_1.assert.isFalse(
        errors_1.HardhatPluginError.isHardhatPluginError(
          new errors_1.HardhatError(
      ...
      chai_1.assert.isFalse(
        errors_1.HardhatPluginError.isHardhatPluginError(undefined)
      );
      chai_1.assert.isFalse(
        errors_1.HardhatPluginError.isHardhatPluginError(null)
      );
      chai_1.assert.isFalse(
        errors_1.HardhatPluginError.isHardhatPluginError(123)
      );
      chai_1.assert.isFalse(
        errors_1.HardhatPluginError.isHardhatPluginError("123")
      );
      chai_1.assert.isFalse(
        errors_1.HardhatPluginError.isHardhatPluginError({ asd: 123 })
      );
    });
  });
...
describe("NomicLabsHardhatPluginError", () => {
  describe("Type guard", () => {
    it("Should return true for NomicLabsHardhatPluginError", () => {
      chai_1.assert.isTrue(
        errors_1.NomicLabsHardhatPluginError.isNomicLabsHardhatPluginError(
          new errors_1.NomicLabsHardhatPluginError("asd", "asd")
        )
      );
    });
    ...
    it("Should return false for everything else", () => {
      ...
      chai_1.assert.isFalse(
        errors_1.NomicLabsHardhatPluginError.isNomicLabsHardhatPluginError(null)
      );
      chai_1.assert.isFalse(
        errors_1.NomicLabsHardhatPluginError.isNomicLabsHardhatPluginError(123)
      );
      chai_1.assert.isFalse(
        errors_1.NomicLabsHardhatPluginError.isNomicLabsHardhatPluginError(
          "123"
...
describe("applyErrorMessageTemplate", () => {
  describe("Variable names", () => {
    it("Should reject invalid variable names", () => {
      (0, errors_2.expectHardhatError)(
        () => (0, errors_1.applyErrorMessageTemplate)("", { 1: 1 }),
        errors_list_1.ERRORS.INTERNAL.TEMPLATE_INVALID_VARIABLE_NAME
      );
      (0, errors_2.expectHardhatError)(
        () => (0, errors_1.applyErrorMessageTemplate)("", { "asd%": 1 }),
        errors_list_1.ERRORS.INTERNAL.TEMPLATE_INVALID_VARIABLE_NAME
      );
      (0, errors_2.expectHardhatError)(
        () => (0, errors_1.applyErrorMessageTemplate)("", { "asd asd": 1 }),
        errors_list_1.ERRORS.INTERNAL.TEMPLATE_INVALID_VARIABLE_NAME
      );
    });
  });
  describe("Values", () => {
    it("shouldn't contain valid variable tags", () => {
      (0, errors_2.expectHardhatError)(
        () => (0, errors_1.applyErrorMessageTemplate)("%asd%", { asd: "%as%" }),
        errors_list_1.ERRORS.INTERNAL.TEMPLATE_VALUE_CONTAINS_VARIABLE_TAG
      );
      (0, errors_2.expectHardhatError)(
        () =>
          (0, errors_1.applyErrorMessageTemplate)("%asd%", { asd: "%a123%" }),
        errors_list_1.ERRORS.INTERNAL.TEMPLATE_VALUE_CONTAINS_VARIABLE_TAG
      );
      (0, errors_2.expectHardhatError)(
        () =>
          (0, errors_1.applyErrorMessageTemplate)("%asd%", {
            asd: { toString: () => "%asd%" },
          }),
        errors_list_1.ERRORS.INTERNAL.TEMPLATE_VALUE_CONTAINS_VARIABLE_TAG
      );
    ...
    it("Shouldn't contain the %% tag", () => {
      (0, errors_2.expectHardhatError)(
        () => (0, errors_1.applyErrorMessageTemplate)("%asd%", { asd: "%%" }),
        errors_list_1.ERRORS.INTERNAL.TEMPLATE_VALUE_CONTAINS_VARIABLE_TAG
      );
    });
  });
  describe("Replacements", () => {
    describe("String values", () => {
      it("Should replace variable tags for the values", () => {
        chai_1.assert.equal(
          (0, errors_1.applyErrorMessageTemplate)("asd %asd% 123 %asd%", {
            asd: "r",
          }),
          "asd r 123 r"
        );
        chai_1.assert.equal(
          (0, errors_1.applyErrorMessageTemplate)("asd%asd% %asd% %fgh% 123", {
            asd: "r",
            fgh: "b",
          }),
          "asdr r b 123"
    ...
    describe("%% sign", () => {
      ...
      it("Shouldn't apply replacements if after this one a new variable tag appears", () => {
        chai_1.assert.equal(
          (0, errors_1.applyErrorMessageTemplate)("asd%%asd%% %asd%", {
            asd: "123",
          }),
          "asd%asd% 123"
        );
    ...
    describe("Missing variable tag", () => {
      it("Should fail if a viable tag is missing and its value is not", () => {
        (0, errors_2.expectHardhatError)(
          () => (0, errors_1.applyErrorMessageTemplate)("", { asd: "123" }),
          errors_list_1.ERRORS.INTERNAL.TEMPLATE_VARIABLE_TAG_MISSING
        );
      });
    });
    describe("Missing variable", () => {
      it("Should work, leaving the variable tag", () => {
        chai_1.assert.equal(
          (0, errors_1.applyErrorMessageTemplate)("%asd% %fgh%", {
            asd: "123",
          }),
          "123 %fgh%"
        );
