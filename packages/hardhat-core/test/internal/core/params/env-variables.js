...
describe("paramNameToEnvVariable", () => {
  it("should convert camelCase to UPPER_CASE and prepend HARDHAT_", () => {
    chai_1.assert.equal(
      (0, env_variables_1.paramNameToEnvVariable)("a"),
      "HARDHAT_A"
    );
    chai_1.assert.equal(
      (0, env_variables_1.paramNameToEnvVariable)("B"),
      "HARDHAT_B"
    );
    chai_1.assert.equal(
      (0, env_variables_1.paramNameToEnvVariable)("AC"),
      "HARDHAT_A_C"
    );
    chai_1.assert.equal(
      (0, env_variables_1.paramNameToEnvVariable)("aC"),
    ...
    chai_1.assert.equal(
      (0, env_variables_1.paramNameToEnvVariable)("camelCaseRight"),
      "HARDHAT_CAMEL_CASE_RIGHT"
    );
    chai_1.assert.equal(
      (0, env_variables_1.paramNameToEnvVariable)("somethingAB"),
...
describe("Env vars arguments parsing", () => {
  it("Should use the default values if arguments are not defined", () => {
    const args = (0, env_variables_1.getEnvHardhatArguments)(
    ...
    chai_1.assert.equal(
      args.help,
      hardhat_params_1.HARDHAT_PARAM_DEFINITIONS.help.defaultValue
    );
    chai_1.assert.equal(
      args.network,
      hardhat_params_1.HARDHAT_PARAM_DEFINITIONS.network.defaultValue
    );
    chai_1.assert.equal(
      args.emoji,
      hardhat_params_1.HARDHAT_PARAM_DEFINITIONS.emoji.defaultValue
    );
    chai_1.assert.equal(
      args.showStackTraces,
      hardhat_params_1.HARDHAT_PARAM_DEFINITIONS.showStackTraces.defaultValue
    );
    chai_1.assert.equal(
      args.version,
      hardhat_params_1.HARDHAT_PARAM_DEFINITIONS.version.defaultValue
    );
  });
  ...
  it("should throw if an invalid value is passed", () => {
    (0, errors_1.expectHardhatError)(
      () =>
        (0, env_variables_1.getEnvHardhatArguments)(
          hardhat_params_1.HARDHAT_PARAM_DEFINITIONS,
          {
            HARDHAT_HELP: "123",
          }
...
describe("getEnvVariablesMap", () => {
  it("Should return the right map", () => {
    chai_1.assert.deepEqual(
      (0, env_variables_1.getEnvVariablesMap)({
        network: "asd",
        emoji: false,
        help: true,
