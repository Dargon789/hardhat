...
const keccak_1 = require("../src/internal/util/keccak");
 ...
 */
function* genPermutations(elemCount, len) {
...
function selector({ name = "", params = [] }) {
  ...
  return (0, keccak_1.keccak256)(Buffer.from(`${name}(${sigParams})`)).slice(
    0,
    4
  );
}
  ...
  [{ name: "log", params: [] }]
    ...
    .concat(
      SINGLE_TYPES.map((single) => {
        const param = Object.assign(Object.assign({}, single), { name: "p0" });
        ...
        return Array.from(
          ...
          (permutation) => ({
            ...
            params: permutation.map((typeIndex, i) =>
              Object.assign(Object.assign({}, TYPES[typeIndex]), {
                name: `p${i}`,
              })
            ),
          })
...
`;
node_fs_1.default.writeFileSync(__dirname + "/../console.sol", consoleSolFile);
