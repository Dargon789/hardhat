...
exports.createProject =
  exports.showSoliditySurveyMessage =
  exports.EMPTY_HARDHAT_CONFIG =
    void 0;
...
(function (Action) {
  ...
  Action["CREATE_EMPTY_HARDHAT_CONFIG_ACTION"] =
  ...
  return __awaiter(this, void 0, void 0, function* () {
    ...
    console.log(
      picocolors_1.default.cyan(
        `${(0, emoji_1.emoji)("👷 ")}Welcome to ${constants_1.HARDHAT_NAME} v${
          packageJson.version
        }${(0, emoji_1.emoji)(" 👷‍")}\n`
      )
    );
...
function copySampleProject(projectRoot, projectType, isEsm) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    yield fs_extra_1.default.ensureDir(projectRoot);
    const sampleProjectPath = path_1.default.join(
      packageRoot,
      "sample-projects",
      sampleProjectName
    );
    // relative paths to all the sample project files
    const sampleProjectFiles = (yield (0, fs_utils_1.getAllFilesMatching)(
      sampleProjectPath
    )).map((file) => path_1.default.relative(sampleProjectPath, file));
    ...
    for (const file of sampleProjectFiles) {
      ...
      if (
        file !== "README.md" &&
        fs_extra_1.default.existsSync(targetProjectFile)
      ) {
        existingFiles.push(file);
      }
    }
    if (existingFiles.length > 0) {
      const errorMsg = `We couldn't initialize the sample project because ${(0,
      strings_1.pluralize)(
...
Please delete or rename ${(0, strings_1.pluralize)(
        existingFiles.length,
        "it",
        "them"
      )} and try again.`;
...
function addGitIgnore(projectRoot) {
  return __awaiter(this, void 0, void 0, function* () {
    const gitIgnorePath = path_1.default.join(projectRoot, ".gitignore");
    let content = yield (0, project_structure_1.getRecommendedGitIgnore)();
    if (yield fs_extra_1.default.pathExists(gitIgnorePath)) {
      const existingContent = yield fs_extra_1.default.readFile(
        gitIgnorePath,
        "utf-8"
      );
...
${content}`;
    ...
    yield fs_extra_1.default.writeFile(gitIgnorePath, content);
  });
}
function printRecommendedDepsInstallationInstructions(projectType) {
  return __awaiter(this, void 0, void 0, function* () {
    console.log(
      `You need to install these dependencies to run the sample project:`
    );
    const cmd = yield getRecommendedDependenciesInstallationCommand(
      yield getDependencies(projectType)
    );
...
`;
function writeEmptyHardhatConfig(isEsm) {
  return __awaiter(this, void 0, void 0, function* () {
    const hardhatConfigFilename = isEsm
      ? "hardhat.config.cjs"
      : "hardhat.config.js";
    return fs_extra_1.default.writeFile(
...
function getAction(isEsm) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    try {
      const actionResponse = yield enquirer.prompt([
        {
          name: "action",
          type: "select",
...
function showStarOnGitHubMessage() {
  ...
  console.log();
  console.log(
    picocolors_1.default.cyan("     https://github.com/NomicFoundation/hardhat")
  );
}
...
function createProject() {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    let packageJson;
    if (yield fs_extra_1.default.pathExists("package.json")) {
      packageJson = yield fs_extra_1.default.readJson("package.json");
    }
    const isEsm =
      (packageJson === null || packageJson === void 0
        ? void 0
        : packageJson.type) === "module";
    const action = yield getAction(isEsm);
    if (action === Action.QUIT_ACTION) {
      return;
    }
    if (isEsm && action === Action.CREATE_TYPESCRIPT_PROJECT_ACTION) {
      throw new errors_1.HardhatError(
        errors_list_1.ERRORS.GENERAL.ESM_TYPESCRIPT_PROJECT_CREATION
      );
    }
    if (packageJson === undefined) {
      yield createPackageJson();
    }
    if (action === Action.CREATE_EMPTY_HARDHAT_CONFIG_ACTION) {
      yield writeEmptyHardhatConfig(isEsm);
      console.log(
        `${(0, emoji_1.emoji)("✨ ")}${picocolors_1.default.cyan(
          `Config file created`
        )}${(0, emoji_1.emoji)(" ✨")}`
      );
      if (!isInstalled(HARDHAT_PACKAGE_NAME)) {
        ...
        const cmd = yield getRecommendedDependenciesInstallationCommand({
          [HARDHAT_PACKAGE_NAME]: `^${
            (yield (0, packageInfo_1.getPackageJson)()).version
          }`,
        });
        console.log("");
        console.log(cmd.join(" "));
    ...
    if (useDefaultPromptResponses) {
      ...
      try {
        responses = yield (0, prompt_1.confirmProjectCreation)();
      } catch (e) {
        if (e === "") {
          return;
        }
    ...
    if (shouldAddGitIgnore) {
      yield addGitIgnore(projectRoot);
    }
    if (
      ...
      yield (0, analytics_1.requestTelemetryConsent)();
    }
    yield copySampleProject(projectRoot, action, isEsm);
    let shouldShowInstallationInstructions = true;
    if (yield canInstallRecommendedDeps()) {
      const dependencies = yield getDependencies(action);
      const recommendedDeps = Object.keys(dependencies);
      const dependenciesToInstall = (0, lang_1.fromEntries)(
      ...
      if (installedRecommendedDeps.length === recommendedDeps.length) {
        ...
        const shouldInstall =
          ...
          (yield (0, prompt_1.confirmRecommendedDepsInstallation)(
            dependenciesToInstall,
            yield getProjectPackageManager()
          ));
        if (shouldInstall) {
          const installed = yield installRecommendedDependencies(
            dependenciesToInstall
          );
          if (!installed) {
            console.warn(
    ...
    if (shouldShowInstallationInstructions) {
      console.log(``);
      yield printRecommendedDepsInstallationInstructions(action);
    }
    console.log(
      `\n${(0, emoji_1.emoji)("✨ ")}${picocolors_1.default.cyan(
        "Project created"
    ...
    showStarOnGitHubMessage();
    showSoliditySurveyMessage();
    const hardhat3BannerManager =
      yield banner_manager_1.BannerManager.getInstance();
    yield hardhat3BannerManager.showBanner();
  });
}
exports.createProject = createProject;
function canInstallRecommendedDeps() {
  return __awaiter(this, void 0, void 0, function* () {
  ...
  const [projectType] =
    (_a =
      userAgent === null || userAgent === void 0
        ? void 0
        : userAgent.split("/")) !== null && _a !== void 0
      ? _a
      : [];
  return projectType;
}
function isYarnProject() {
  return __awaiter(this, void 0, void 0, function* () {
    return (
      getProjectTypeFromUserAgent() === "yarn" ||
      fs_extra_1.default.pathExists("yarn.lock")
    );
  });
}
function isPnpmProject() {
  return __awaiter(this, void 0, void 0, function* () {
    return (
      ...
      fs_extra_1.default.pathExists("pnpm-lock.yaml")
    );
  });
}
function getProjectPackageManager() {
  return __awaiter(this, void 0, void 0, function* () {
    if (yield isYarnProject()) return "yarn";
    if (yield isPnpmProject()) return "pnpm";
    return "npm";
  });
}
function doesNpmAutoInstallPeerDependencies() {
  return __awaiter(this, void 0, void 0, function* () {
...
function installRecommendedDependencies(dependencies) {
  return __awaiter(this, void 0, void 0, function* () {
    console.log("");
    const installCmd = yield getRecommendedDependenciesInstallationCommand(
      dependencies
    );
    return installDependencies(installCmd[0], installCmd.slice(1));
  });
}
function installDependencies(packageManager, args) {
  return __awaiter(this, void 0, void 0, function* () {
    const { spawn } = yield Promise.resolve().then(() =>
...
function getRecommendedDependenciesInstallationCommand(dependencies) {
  return __awaiter(this, void 0, void 0, function* () {
    const deps = Object.entries(dependencies).map(
      ([name, version]) => `"${name}@${version}"`
    );
    if (yield isYarnProject()) {
      return ["yarn", "add", "--dev", ...deps];
    }
    if (yield isPnpmProject()) {
...
function getDependencies(projectType) {
  return __awaiter(this, void 0, void 0, function* () {
    const shouldInstallPeerDependencies =
      (yield isYarnProject()) ||
      (yield isPnpmProject()) ||
      !(yield doesNpmAutoInstallPeerDependencies());
    ...
    const shouldInstallDefaultToolbox =
      projectType !== Action.CREATE_TYPESCRIPT_VIEM_PROJECT_ACTION;
    const ethersToolboxDependencies = Object.assign(
      Object.assign(
        Object.assign({}, ETHERS_PROJECT_DEPENDENCIES),
    ...
    const viemToolboxDependencies = Object.assign(
      Object.assign(
        Object.assign({}, VIEM_PROJECT_DEPENDENCIES),
    ...
    return Object.assign(
      Object.assign({}, commonDependencies),
      toolboxDependencies
    );
  });
}
