...
const SHOULD_SHOW_STACK_TRACES_BY_DEFAULT = (0,
...
function suggestInstallingHardhatVscode() {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    const isInstalled = (0,
    hardhat_vscode_installation_1.isHardhatVSCodeInstalled)();
    (0, global_dir_1.writePromptedForHHVSCode)();
    if (
      isInstalled !==
      hardhat_vscode_installation_1.InstallationState.EXTENSION_NOT_INSTALLED
    ) {
      return;
    }
    const installationConsent = yield (0,
    prompt_1.confirmHHVSCodeInstallation)();
    if (installationConsent === true) {
      console.log("Installing Hardhat for Visual Studio Code...");
      const installed = (0,
      hardhat_vscode_installation_1.installHardhatVSCode)();
      if (installed) {
        console.log(
          "Hardhat for Visual Studio Code was successfully installed"
        );
      } else {
        console.log(
...
function showViaIRWarning(resolvedConfig) {
  const configuredCompilers = (0, config_loading_1.getConfiguredCompilers)(
    resolvedConfig.solidity
  );
  const viaIREnabled = configuredCompilers.some((compiler) => {
    var _a;
    return (
      ((_a = compiler.settings) === null || _a === void 0
        ? void 0
        : _a.viaIR) === true
    );
  });
  if (viaIREnabled) {
    ...
    console.warn(
      picocolors_1.default
        .yellow(`Your solidity settings have viaIR enabled, which is not fully supported yet. You can still use Hardhat, but some features, like stack traces, might not work correctly.
...
function main() {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    try {
      const envVariableArguments = (0, env_variables_1.getEnvHardhatArguments)(
      ...
      // #INIT-DEP - START OF DEPRECATED CODE
        ...
        if (
          ...
          console.warn(
            ...
            picocolors_1.default.yellow(
              `Initializing a project with ${picocolors_1.default.white(
                picocolors_1.default.italic("npx hardhat")
              )} is deprecated and will be removed in the future.\n`
            ),
            picocolors_1.default.yellow(
              `Please use ${picocolors_1.default.white(
                picocolors_1.default.italic("npx hardhat init")
              )} instead.\n\n`
            )
          );
          return;
      ...
      if (
        ...
        throw new errors_1.HardhatError(
          errors_list_1.ERRORS.GENERAL.NON_LOCAL_INSTALLATION
        );
      }
      if (
        (0, typescript_support_1.willRunWithTypescript)(hardhatArguments.config)
      ) {
        (0, typescript_support_1.loadTsNode)(
          hardhatArguments.tsconfig,
          hardhatArguments.typecheck
        );
      } else {
        if (hardhatArguments.typecheck === true) {
          throw new errors_1.HardhatError(
            errors_list_1.ERRORS.ARGUMENTS.TYPECHECK_USED_IN_JAVASCRIPT_PROJECT
          );
        }
      }
      ...
      if (scopeOrTaskName === "vars" && allUnparsedCLAs.length > 1) {
        process.exit(
          yield (0, vars_1.handleVars)(allUnparsedCLAs, hardhatArguments.config)
        );
      }
      const { resolvedConfig, userConfig } = (0,
      config_loading_1.loadConfigAndTasks)(hardhatArguments, {
        showEmptyConfigWarning: true,
        showSolidityConfigWarnings:
          scopeOrTaskName === task_names_1.TASK_COMPILE,
      });
      ...
      const env = new runtime_environment_1.Environment(
        resolvedConfig,
        hardhatArguments,
        taskDefinitions,
        scopesDefinitions,
      ...
      const isHelpCommand =
        hardhatArguments.help || taskName === task_names_1.TASK_HELP;
      if (
        ...
        telemetryConsent = yield (0, analytics_1.requestTelemetryConsent)();
      }
      const analytics = yield analytics_1.Analytics.getInstance(
        telemetryConsent
      );
      ...
      const [abortAnalytics, hitPromise] = yield analytics.sendTaskHit(
        scopeName,
        taskName
      );
      let taskArguments;
      ...
      if (hardhatArguments.help && taskName !== task_names_1.TASK_HELP) {
        ...
        if (taskDefinition === undefined) {
          if (scopeName !== undefined) {
            throw new errors_1.HardhatError(
              errors_list_1.ERRORS.ARGUMENTS.UNRECOGNIZED_SCOPED_TASK,
              {
                scope: scopeName,
                task: taskName,
              }
          ...
          throw new errors_1.HardhatError(
            errors_list_1.ERRORS.ARGUMENTS.UNRECOGNIZED_TASK,
            {
              task: taskName,
            }
          );
        ...
        if (taskDefinition.isSubtask) {
          throw new errors_1.HardhatError(
            errors_list_1.ERRORS.ARGUMENTS.RUNNING_SUBTASK_FROM_CLI,
            {
      ...
      try {
        ...
        yield env.run({ scope: scopeName, task: taskName }, taskArguments);
        ...
        if (hardhatArguments.flamegraph === true) {
          ...
          const flamegraphPath = (0, flamegraph_1.saveFlamegraph)(
            env.entryTaskProfile
          );
      ...
      if (
        taskName === task_names_1.TASK_TEST &&
        !(0, ci_detection_1.isRunningOnCiServer)() &&
        process.stdout.isTTY === true
      ) {
        yield suggestInstallingHardhatVscode();
        ...
        if (process.exitCode !== 0) {
          try {
            const { showNewVersionNotification } = yield Promise.resolve().then(
              () => __importStar(require("./version-notifier"))
            );
            yield showNewVersionNotification();
          } catch (_a) {
      ...
      if (
        ...
        yield hardhat3BannerManager.showBanner(200);
      }
      ...
      if (errors_1.HardhatError.isHardhatError(error)) {
        ...
        console.error(
          ...
          error.message.replace(/^\w+:/, (t) =>
            picocolors_1.default.red(picocolors_1.default.bold(t))
          )
        );
        ...
        console.error(
          picocolors_1.default.red(
            picocolors_1.default.bold(`Error in plugin ${error.pluginName}:`)
          ),
          error.message
        );
      } else if (error instanceof Error) {
        console.error(
          picocolors_1.default.red("An unexpected error occurred:")
        );
      ...
      if (showStackTraces || SHOULD_SHOW_STACK_TRACES_BY_DEFAULT) {
        ...
        if (errors_1.HardhatError.isHardhatError(error)) {
          const link = `https://hardhat.org/${(0, errors_list_1.getErrorCode)(
            error.errorDescriptor
          )}`;
          console.error(
            `For more info go to ${link} or run ${constants_1.HARDHAT_NAME} with --show-stack-traces`
          );
        } else {
          console.error(
            `For more info run ${constants_1.HARDHAT_NAME} with --show-stack-traces`
          );
        }
      }
      yield reporter_1.Reporter.close(1000);
      process.exit(1);
    }
  });
}
function createNewProject() {
  return __awaiter(this, void 0, void 0, function* () {
    if ((0, project_structure_1.isCwdInsideProject)()) {
      throw new errors_1.HardhatError(
        errors_list_1.ERRORS.GENERAL.HARDHAT_PROJECT_ALREADY_CREATED,
        {
    ...
    if (
      ...
      yield (0, project_creation_1.createProject)();
      return;
    }
    ...
    if (process.platform === "win32") {
      throw new errors_1.HardhatError(
        errors_list_1.ERRORS.GENERAL.NOT_INSIDE_PROJECT_ON_WINDOWS
      );
    }
    throw new errors_1.HardhatError(
      errors_list_1.ERRORS.GENERAL.NOT_IN_INTERACTIVE_SHELL
    );
  });
}
main()
