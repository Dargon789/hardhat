...
function handleVars(allUnparsedCLAs, configPath) {
  return __awaiter(this, void 0, void 0, function* () {
    const { taskDefinition, taskArguments } =
      yield getTaskDefinitionAndTaskArguments(allUnparsedCLAs);
    switch (taskDefinition.name) {
      ...
      default:
        console.error(
          picocolors_1.default.red(`Invalid task '${taskDefinition.name}'`)
        );
        return 1; // Error code
    }
  });
...
function set(key, value) {
  return __awaiter(this, void 0, void 0, function* () {
    const varsManager =
      context_1.HardhatContext.getHardhatContext().varsManager;
    varsManager.validateKey(key);
    varsManager.set(
      key,
      value !== null && value !== void 0 ? value : yield getVarValue()
    );
...
function get(key) {
  const value =
    context_1.HardhatContext.getHardhatContext().varsManager.get(key);
  if (value !== undefined) {
    console.log(value);
    return 0;
  }
  console.warn(
    picocolors_1.default.yellow(
      `The configuration variable '${key}' is not set in ${context_1.HardhatContext.getHardhatContext().varsManager.getStoragePath()}`
    )
  );
  return 1;
}
function list() {
  ...
  const varsStoragePath =
    context_1.HardhatContext.getHardhatContext().varsManager.getStoragePath();
  if (keys.length > 0) {
...
function setup(configPath) {
  ...
  const varsManagerSetup = new vars_manager_setup_1.VarsManagerSetup(
    (0, global_dir_1.getVarsFilePath)()
  );
...
function loadConfigFile(configPath) {
  ...
  const resolvedConfigPath = (0, config_loading_1.resolveConfigPath)(
    configPath
  );
...
function getVarValue() {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    const response = yield enquirer.prompt({
      type: "password",
      name: "value",
      message: "Enter value:",
    });
...
function printAlreadySetKeys(varsManagerSetup) {
  ...
  console.log(
    `${picocolors_1.default.bold(
      `${(0, emoji_1.emoji)("✔️  ")}Configuration variables already set:`
    )}`
  );
  console.log();
...
function getTaskDefinitionAndTaskArguments(allUnparsedCLAs) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    (0,
    errors_1.assertHardhatInvariant)(scopeName === "vars", "This function should only be called to handle tasks under the 'vars' scope");
