...
(function (CompilerPlatform) {
  ...
  (CompilerPlatform =
    exports.CompilerPlatform || (exports.CompilerPlatform = {}))
);
/**
...
class CompilerDownloader {
  ...
  constructor(
    ...
    _downloadFunction = download_1.download
  ) {
    ...
    this._mutex = new multi_process_mutex_1.MultiProcessMutex(
      "compiler-download"
    );
  }
  isCompilerDownloaded(version) {
    return __awaiter(this, void 0, void 0, function* () {
      const build = yield this._getCompilerBuild(version);
      if (build === undefined) {
        return false;
      }
  ...
  downloadCompiler(version, downloadStartedCb, downloadEndedCb) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      yield this._mutex.use(() =>
        __awaiter(this, void 0, void 0, function* () {
          const isCompilerDownloaded = yield this.isCompilerDownloaded(version);
          if (isCompilerDownloaded === true) {
            return;
          }
          yield downloadStartedCb(isCompilerDownloaded);
          let build = yield this._getCompilerBuild(version);
          if (
            build === undefined &&
            (yield this._shouldDownloadCompilerList())
          ) {
            try {
              yield this._downloadCompilerList();
            } catch (e) {
            ...
            build = yield this._getCompilerBuild(version);
          }
          if (build === undefined) {
            throw new errors_1.HardhatError(
          ...
          let downloadPath;
          try {
            downloadPath = yield this._downloadCompiler(build);
          } catch (e) {
            throw new errors_1.HardhatError(
          ...
          const verified = yield this._verifyCompilerDownload(
            build,
            downloadPath
          );
          if (!verified) {
          ...
          yield this._postProcessCompilerDownload(build, downloadPath);
          yield downloadEndedCb(isCompilerDownloaded);
        })
      );
    });
  }
  getCompiler(version) {
    return __awaiter(this, void 0, void 0, function* () {
      const build = yield this._getCompilerBuild(version);
      (0,
      errors_1.assertHardhatInvariant)(build !== undefined, "Trying to get a compiler before it was downloaded");
      const compilerPath = this._getCompilerBinaryPathFromBuild(build);
      (0,
      errors_1.assertHardhatInvariant)(yield fs_extra_1.default.pathExists(compilerPath), "Trying to get a compiler before it was downloaded");
      if (
        yield fs_extra_1.default.pathExists(
          this._getCompilerDoesntWorkFile(build)
        )
      ) {
        return undefined;
      }
  ...
  _getCompilerBuild(version) {
    return __awaiter(this, void 0, void 0, function* () {
      const listPath = this._getCompilerListPath();
      if (!(yield fs_extra_1.default.pathExists(listPath))) {
        return undefined;
      }
      const list = yield this._readCompilerList(listPath);
  ...
  _shouldDownloadCompilerList() {
    return __awaiter(this, void 0, void 0, function* () {
      const listPath = this._getCompilerListPath();
      if (!(yield fs_extra_1.default.pathExists(listPath))) {
        return true;
      }
      const stats = yield fs_extra_1.default.stat(listPath);
      const age = new Date().valueOf() - stats.ctimeMs;
      return age > this._compilerListCachePeriodMs;
    });
  }
  _downloadCompilerList() {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      yield this._downloadFunction(url, downloadPath);
    });
  }
  _downloadCompiler(build) {
    return __awaiter(this, void 0, void 0, function* () {
  ...
  _verifyCompilerDownload(build, downloadPath) {
    return __awaiter(this, void 0, void 0, function* () {
      const { bytesToHex } = require("@ethereumjs/util");
      const { keccak256 } = yield Promise.resolve().then(() =>
        __importStar(require("../../util/keccak"))
      );
      const expectedKeccak256 = build.keccak256;
      const compiler = yield fs_extra_1.default.readFile(downloadPath);
      const compilerKeccak256 = bytesToHex(keccak256(compiler));
      if (expectedKeccak256 !== compilerKeccak256) {
        yield fs_extra_1.default.unlink(downloadPath);
        return false;
      }
      return true;
    });
  }
  _postProcessCompilerDownload(build, downloadPath) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      if (
        ...
        const solcFolder = path_1.default.join(
          this._compilersDir,
          build.version
        );
        yield fs_extra_1.default.ensureDir(solcFolder);
        const zip = new AdmZip(downloadPath);
        zip.extractAllTo(solcFolder);
      }
      log("Checking native solc binary");
      const nativeSolcWorks = yield this._checkNativeSolc(build);
      if (nativeSolcWorks) {
        return;
      }
      yield fs_extra_1.default.createFile(
        this._getCompilerDoesntWorkFile(build)
      );
    });
  }
  _checkNativeSolc(build) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      try {
        yield execFileP(solcPath, ["--version"]);
        return true;
      } catch (_a) {
        return false;
      }
