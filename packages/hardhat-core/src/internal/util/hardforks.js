...
exports.selectHardfork =
  exports.hardforkGte =
  exports.getHardforkName =
  exports.HardforkName =
    void 0;
...
function getHardforkName(name) {
  const hardforkName =
    Object.values(HardforkName)[Object.values(HardforkName).indexOf(name)];
...
function selectHardfork(
  ...
  if (hardforkActivations === undefined || hardforkActivations.size === 0) {
    throw new errors_2.InternalError(
  ...
  if (!constants_1.HARDHAT_NETWORK_SUPPORTED_HARDFORKS.includes(hardfork)) {
    throw new errors_2.InternalError(
      `Tried to run a call or transaction in the context of a block whose hardfork is "${hardfork}", but Hardhat Network only supports the following hardforks: ${constants_1.HARDHAT_NETWORK_SUPPORTED_HARDFORKS.join(
        ", "
      )}`
    );
  }
