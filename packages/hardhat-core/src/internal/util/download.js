...
function download(url, filePath, timeoutMillis = 10000, extraHeaders = {}) {
  return __awaiter(this, void 0, void 0, function* () {
    const { getGlobalDispatcher, ProxyAgent, request } =
      yield Promise.resolve().then(() => __importStar(require("undici")));
    let dispatcher;
    if (
      process.env.http_proxy !== undefined &&
      (0, proxy_1.shouldUseProxy)(url)
    ) {
    ...
    const response = yield request(url, {
      ...
      maxRedirections: 10,
      method: "GET",
      headers: Object.assign(Object.assign({}, extraHeaders), {
        "User-Agent": `hardhat ${hardhatVersion}`,
      }),
    });
    if (response.statusCode >= 200 && response.statusCode <= 299) {
      const responseBody = Buffer.from(yield response.body.arrayBuffer());
      const tmpFilePath = resolveTempFileName(filePath);
      yield fs_extra_1.default.ensureDir(path_1.default.dirname(filePath));
      yield fs_extra_1.default.writeFile(tmpFilePath, responseBody);
