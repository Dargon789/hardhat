...
class HardhatContext {
  constructor() {
    ...
    this.varsManager = new vars_manager_1.VarsManager(
  ...
  static createHardhatContext() {
    if (this.isCreated()) {
      throw new errors_1.HardhatError(
        errors_list_1.ERRORS.GENERAL.CONTEXT_ALREADY_CREATED
      );
    }
    const globalWithHardhatContext = global;
  ...
  static getHardhatContext() {
    ...
    if (ctx === undefined) {
      throw new errors_1.HardhatError(
        errors_list_1.ERRORS.GENERAL.CONTEXT_NOT_CREATED
      );
    }
    return ctx;
  }
  ...
  setHardhatRuntimeEnvironment(env) {
    if (this.environment !== undefined) {
      throw new errors_1.HardhatError(
        errors_list_1.ERRORS.GENERAL.CONTEXT_HRE_ALREADY_DEFINED
      );
    }
  ...
  getHardhatRuntimeEnvironment() {
    if (this.environment === undefined) {
      throw new errors_1.HardhatError(
        errors_list_1.ERRORS.GENERAL.CONTEXT_HRE_NOT_DEFINED
      );
    }
