...
class VarsManager {
  ...
  set(key, value) {
    ...
    if (value === "") {
      throw new errors_1.HardhatError(
        errors_list_1.ERRORS.VARS.INVALID_EMPTY_VALUE
      );
    }
  ...
  get(key, defaultValue, includeEnvs = false) {
    ...
    return (_b =
      (_a = this._storageCache.vars[key]) === null || _a === void 0
        ? void 0
  ...
  validateKey(key) {
    ...
    if (!KEY_REGEX.test(key)) {
      throw new errors_1.HardhatError(
        errors_list_1.ERRORS.VARS.INVALID_CONFIG_VAR_NAME,
        {
          value: key,
        }
      );
