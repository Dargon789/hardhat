...
class HttpProvider extends events_1.EventEmitter {
  constructor(
    ...
    try {
      this._dispatcher =
        client !== null && client !== void 0 ? client : new Pool(url.origin);
      if (
        process.env.http_proxy !== undefined &&
        (0, proxy_1.shouldUseProxy)(url.origin)
      ) {
  ...
  request(args) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      if (args.method === "hardhat_setLedgerOutputEnabled") {
        const error = new errors_2.ProviderError(
      ...
      const jsonRpcResponse = yield this._fetchJsonRpcResponse(jsonRpcRequest);
  ...
  sendBatch(batch) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const stackSavingError = new errors_2.ProviderError(
        "HttpProviderError",
        -1
      );
      ...
      const jsonRpcResponses = yield this._fetchJsonRpcResponse(requests);
  ...
  _fetchJsonRpcResponse(request, retryNumber = 0) {
    return __awaiter(this, void 0, void 0, function* () {
      const { request: sendRequest } = yield Promise.resolve().then(() =>
      ...
      try {
        ...
        if (this._isRateLimitResponse(response)) {
          ...
          yield response.body.text();
          const seconds = this._getRetryAfterSeconds(response, retryNumber);
          if (
            seconds !== undefined &&
            this._shouldRetry(retryNumber, seconds)
          ) {
            return yield this._retry(request, seconds, retryNumber);
          }
          // eslint-disable-next-line @nomicfoundation/hardhat-internal-rules/only-hardhat-error
          throw new errors_2.ProviderError(
            `Too Many Requests error received from ${url.hostname}`,
            -32005 // Limit exceeded according to EIP1474
          );
        }
        return (0, jsonrpc_1.parseJsonResponse)(yield response.body.text());
      } catch (error) {
        if (error.code === "ECONNREFUSED") {
          throw new errors_1.HardhatError(
            errors_list_1.ERRORS.NETWORK.NODE_IS_NOT_RUNNING,
        ...
        if (error.type === "request-timeout") {
          throw new errors_1.HardhatError(
            errors_list_1.ERRORS.NETWORK.NETWORK_TIMEOUT,
            {},
            error
          );
        }
  ...
  _retry(request, seconds, retryNumber) {
    return __awaiter(this, void 0, void 0, function* () {
      yield new Promise((resolve) => setTimeout(resolve, 1000 * seconds));
