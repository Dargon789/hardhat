...
exports.getConfiguredCompilers =
  exports.analyzeModuleNotFoundError =
  exports.loadConfigAndTasks =
  exports.resolveConfigPath =
  exports.importCsjOrEsModule =
    void 0;
  ...
  try {
    ...
    if (node20Heuristic || node22Heuristic) {
      throw new errors_1.HardhatError(
        errors_list_1.ERRORS.GENERAL.ESM_PROJECT_WITHOUT_CJS_CONFIG,
        {},
        e
      );
    }
...
function loadConfigAndTasks(
  hardhatArguments,
  { showEmptyConfigWarning = false, showSolidityConfigWarnings = false } = {
  ...
  const resolved = (0, config_resolution_1.resolveConfig)(
    configPath,
    userConfig
  );
  for (const extender of context_1.HardhatContext.getHardhatContext()
    .configExtenders) {
...
function deepFreezeUserConfig(config, propertyPath = []) {
  ...
  return new Proxy(config, {
    ...
    set(target, property, _value, _receiver) {
      throw new errors_1.HardhatError(
        errors_list_1.ERRORS.GENERAL.USER_CONFIG_MODIFIED,
        {
...
function analyzeModuleNotFoundError(error, configPath) {
  ...
  const throwingFile = stackTrace
    ...
    .filter(
      (x) =>
        !x.includes(path_1.default.join("@cspotcode", "source-map-support"))
    )
  ...
  const packageJsonPath = (0, packageInfo_1.findClosestPackageJson)(
    throwingFile
  );
  ...
  const peerDependencies =
    (_a = packageJson.peerDependencies) !== null && _a !== void 0 ? _a : {};
  if (peerDependencies["@nomiclabs/buidler"] !== undefined) {
    throw new errors_1.HardhatError(
      errors_list_1.ERRORS.PLUGINS.BUIDLER_PLUGIN,
      {
  ...
  if (missingPeerDependenciesNames.length > 0) {
    throw new errors_1.HardhatError(
      errors_list_1.ERRORS.PLUGINS.MISSING_DEPENDENCIES,
      {
        plugin: packageJson.name,
...
function readPackageJson(packageName, configPath) {
  ...
  try {
    const packageJsonPath = resolve.sync(
      path_1.default.join(packageName, "package.json"),
      {
  ...
  if (userConfig.solidity === undefined) {
    console.warn(
      picocolors_1.default
        .yellow(`Solidity compiler is not configured. Version ${default_config_1.DEFAULT_SOLC_VERSION} will be used by default. Add a 'solidity' entry to your configuration to suppress this warning.
...
function checkUnsupportedSolidityConfig(resolvedConfig) {
  ...
  for (const solcVersion of solcVersions) {
    if (
      !semver_1.default.satisfies(
        solcVersion,
        constants_1.SUPPORTED_SOLIDITY_VERSION_RANGE
      ) &&
  ...
  if (unsupportedVersions.length > 0) {
    console.warn(
      picocolors_1.default.yellow(`Solidity ${unsupportedVersions.join(", ")} ${
...
function checkUnsupportedRemappings({ solidity }) {
  ...
  if (remappings.length > 0) {
    console.warn(
      picocolors_1.default
        .yellow(`Solidity remappings are not currently supported; you may experience unexpected compilation results. Remove any 'remappings' fields from your configuration to suppress this warning.
