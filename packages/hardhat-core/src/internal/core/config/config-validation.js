...
exports.validateResolvedConfig =
  exports.getValidationErrors =
  exports.validateConfig =
  exports.decimalString =
  exports.address =
  exports.hexString =
...
exports.address = new t.Type(
  "address",
  isAddress,
  (u, c) => (isAddress(u) ? t.success(u) : t.failure(u, c)),
  t.identity
);
exports.decimalString = new t.Type(
  "decimal string",
  isDecimalString,
...
const HardhatNetworkMempoolConfig = t.type({
  order: (0, io_ts_1.optional)(
    t.keyof(
      (0, lang_1.fromEntries)(
        constants_1.HARDHAT_MEMPOOL_SUPPORTED_ORDERS.map((order) => [
          order,
          null,
        ])
      )
...
const HardhatNetworkMiningConfig = t.type({
  ...
  interval: (0, io_ts_1.optional)(
    t.union([t.number, t.tuple([t.number, t.number])])
  ),
...
const HardforkNameType = new t.Type(
  Object.values(hardforks_1.HardforkName)
    .map((v) => `"${v}"`)
    .join(" | "),
  (name) => typeof name === "string" && isValidHardforkName(name),
  (u, c) => {
...
const HardhatNetworkConfig = t.type(
  Object.assign(Object.assign({}, commonNetworkConfigFields), {
    hardfork: (0, io_ts_1.optional)(
      t.keyof(
        (0, lang_1.fromEntries)(
          constants_1.HARDHAT_NETWORK_SUPPORTED_HARDFORKS.map((hf) => [
            hf,
            null,
          ])
        )
...
const HDAccountsConfig = t.type(
  Object.assign(
    { mnemonic: t.string, passphrase: (0, io_ts_1.optional)(t.string) },
    commonHDAccountsFields
  )
);
  ...
  if (config !== undefined && typeof config.networks === "object") {
    ...
    if (hardhatNetwork !== undefined && typeof hardhatNetwork === "object") {
      if ("url" in hardhatNetwork) {
        errors.push(
          `HardhatConfig.networks.${constants_1.HARDHAT_NETWORK_NAME} can't have a url`
        );
      }
      // Validating the accounts with io-ts leads to very confusing errors messages
      const { accounts } = hardhatNetwork,
      ...
      if (netConfigResult.isLeft()) {
        errors.push(
          getErrorMessage(
            `HardhatConfig.networks.${constants_1.HARDHAT_NETWORK_NAME}`,
            hardhatNetwork,
      ...
      if (Array.isArray(accounts)) {
        for (const [index, account] of accounts.entries()) {
          ...
          if (typeof balance !== "string") {
            errors.push(
              getErrorMessage(
                `HardhatConfig.networks.${constants_1.HARDHAT_NETWORK_NAME}.accounts[].balance`,
                balance,
                "string"
              )
            );
        ...
        if (hdConfigResult.isLeft()) {
          errors.push(
            getErrorMessage(
              `HardhatConfig.networks.${constants_1.HARDHAT_NETWORK_NAME}.accounts`,
              hardhatNetwork.accounts,
      ...
      const hardfork =
        (_a = hardhatNetwork.hardfork) !== null && _a !== void 0
          ? _a
          : default_config_1.defaultHardhatNetworkParams.hardfork;
      if (
        (0, hardforks_1.hardforkGte)(hardfork, hardforks_1.HardforkName.LONDON)
      ) {
        if (hardhatNetwork.minGasPrice !== undefined) {
          errors.push(
            `Unexpected config HardhatConfig.networks.${constants_1.HARDHAT_NETWORK_NAME}.minGasPrice found - This field is not valid for networks with EIP-1559. Try an older hardfork or remove it.`
          );
        }
      } else {
        if (hardhatNetwork.initialBaseFeePerGas !== undefined) {
          errors.push(
            `Unexpected config HardhatConfig.networks.${constants_1.HARDHAT_NETWORK_NAME}.initialBaseFeePerGas found - This field is only valid for networks with EIP-1559. Try a newer hardfork or remove it.`
          );
        }
      }
        ...
        Object.entries(hardhatNetwork.chains).forEach((chainEntry) => {
          ...
          if (hardforkHistory !== undefined) {
            Object.keys(hardforkHistory).forEach((hardforkName) => {
              if (
                !constants_1.HARDHAT_NETWORK_SUPPORTED_HARDFORKS.includes(
                  hardforkName
                )
              ) {
                errors.push(
                  getErrorMessage(
                    `HardhatConfig.networks.${constants_1.HARDHAT_NETWORK_NAME}.chains[${chainId}].hardforkHistory`,
                    hardforkName,
                    `"${constants_1.HARDHAT_NETWORK_SUPPORTED_HARDFORKS.join(
                      '" | "'
                    )}"`
                  )
                );
      ...
      if (hardhatNetwork.hardfork !== undefined) {
        if (
          !(0, hardforks_1.hardforkGte)(
            hardhatNetwork.hardfork,
            hardforks_1.HardforkName.CANCUN
          ) &&
      ...
      return (
        ((_a =
          settings === null || settings === void 0
            ? void 0
            : settings.optimizer) === null || _a === void 0
          ? void 0
          : _a.runs) !== undefined
      );
    })
