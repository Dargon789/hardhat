...
(0, config_env_1.subtask)(task_names_1.TASK_TEST_GET_TEST_FILES)
  ...
  .setAction(({ testFiles }, { config }) =>
    __awaiter(void 0, void 0, void 0, function* () {
      if (testFiles.length !== 0) {
        const testFilesAbsolutePaths = testFiles.map((x) =>
          path_1.default.resolve(process.cwd(), x)
        );
        return testFilesAbsolutePaths;
      }
      const jsFiles = yield (0, fs_utils_1.getAllFilesMatching)(
        config.paths.tests,
        typescript_support_1.isJavascriptFile
      );
      if (!(0, typescript_support_1.isRunningWithTypescript)(config)) {
        return jsFiles;
      }
      const tsFiles = yield (0, fs_utils_1.getAllFilesMatching)(
        config.paths.tests,
        typescript_support_1.isTypescriptFile
      );
      return [...jsFiles, ...tsFiles];
    })
  );
(0, config_env_1.subtask)(task_names_1.TASK_TEST_SETUP_TEST_ENVIRONMENT, () =>
  __awaiter(void 0, void 0, void 0, function* () {})
);
...
(0, config_env_1.subtask)(task_names_1.TASK_TEST_RUN_MOCHA_TESTS)
  ...
  .setAction((taskArgs, { config }) =>
    __awaiter(void 0, void 0, void 0, function* () {
      var _a;
      const { default: Mocha } = yield Promise.resolve().then(() =>
        __importStar(require("mocha"))
      );
      ...
      if (mochaConfig.parallel === true) {
        const mochaRequire =
          (_a = mochaConfig.require) !== null && _a !== void 0 ? _a : [];
      ...
      const projectPackageJson = yield (0,
      packageInfo_1.getProjectPackageJson)();
...
(0, config_env_1.subtask)(
  task_names_1.TASK_TEST_RUN_SHOW_FORK_RECOMMENDATIONS
).setAction((_, { config, network }) =>
  __awaiter(void 0, void 0, void 0, function* () {
    ...
    yield (0,
    fork_recomendations_banner_1.showForkRecommendationsBannerIfNecessary)(network.config, forkCache);
  })
);
(0, config_env_1.task)(task_names_1.TASK_TEST, "Runs mocha tests")
  ...
  .setAction(
    ({ testFiles, noCompile, parallel, bail, grep }, { run, network }) =>
      __awaiter(void 0, void 0, void 0, function* () {
        if (!noCompile) {
          yield run(task_names_1.TASK_COMPILE, { quiet: true });
        }
        const files = yield run(task_names_1.TASK_TEST_GET_TEST_FILES, {
          testFiles,
        });
        yield run(task_names_1.TASK_TEST_SETUP_TEST_ENVIRONMENT);
        yield run(task_names_1.TASK_TEST_RUN_SHOW_FORK_RECOMMENDATIONS);
        const testFailures = yield run(task_names_1.TASK_TEST_RUN_MOCHA_TESTS, {
          testFiles: files,
          parallel,
          bail,
          grep,
        ...
        if (network.name === constants_1.HARDHAT_NETWORK_NAME) {
          const stackTracesFailures = yield network.provider.send(
            "hardhat_getStackTraceFailuresCount"
          );
          if (stackTracesFailures !== 0) {
            console.warn(
              picocolors_1.default.yellow(
                `Failed to generate ${stackTracesFailures} ${(0,
                strings_1.pluralize)(
                  stackTracesFailures,
                  "stack trace"
