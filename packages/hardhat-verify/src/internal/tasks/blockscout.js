...
(0, config_1.subtask)(task_names_1.TASK_VERIFY_BLOCKSCOUT)
    ...
    .addFlag("force")
    .setAction((taskArgs, { config: config, network: network, run }) => __awaiter(void 0, void 0, void 0, function* () {
    const { address, libraries, contractFQN, force } = yield run(task_names_1.TASK_VERIFY_BLOCKSCOUT_RESOLVE_ARGUMENTS, taskArgs);
    const chainConfig = yield blockscout_1.Blockscout.getCurrentChainConfig(network.name, network.provider, config.blockscout.customChains);
    const blockscout = blockscout_1.Blockscout.fromChainConfig(chainConfig);
    const isVerified = yield blockscout.isVerified(address);
    if (!force && isVerified) {
...
`);
    ...
    const minimalInput = yield run(task_names_1.TASK_VERIFY_ETHERSCAN_GET_MINIMAL_INPUT, {
        sourceName: contractInformation.sourceName,
    });
    // First, try to verify the contract using the minimal input
    const { success: minimalInputVerificationSuccess } = yield run(task_names_1.TASK_VERIFY_BLOCKSCOUT_ATTEMPT_VERIFICATION, {
        address,
...
`);
    ...
    const { success: fullCompilerInputVerificationSuccess, message: verificationMessage, } = yield run(task_names_1.TASK_VERIFY_BLOCKSCOUT_ATTEMPT_VERIFICATION, {
        address,
...
(0, config_1.subtask)(task_names_1.TASK_VERIFY_BLOCKSCOUT_RESOLVE_ARGUMENTS)
    ...
    .addOptionalParam("contract")
    .addFlag("force")
    .setAction(({ address, contract, libraries: librariesModule, force, }) => __awaiter(void 0, void 0, void 0, function* () {
...
(0, config_1.subtask)(task_names_1.TASK_VERIFY_BLOCKSCOUT_ATTEMPT_VERIFICATION)
    ...
    .setAction(({ address, compilerInput, contractInformation, verificationInterface, }, { run }) => __awaiter(void 0, void 0, void 0, function* () {
