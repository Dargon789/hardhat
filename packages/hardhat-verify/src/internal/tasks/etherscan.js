...
(0, config_1.subtask)(task_names_2.TASK_VERIFY_ETHERSCAN)
    ...
    const { address, constructorArgs, libraries, contractFQN, force, } = yield run(task_names_2.TASK_VERIFY_ETHERSCAN_RESOLVE_ARGUMENTS, taskArgs);
    const chainConfig = yield etherscan_1.Etherscan.getCurrentChainConfig(network.name, network.provider, config.etherscan.customChains);
    const etherscan = etherscan_1.Etherscan.fromChainConfig(config.etherscan.apiKey, chainConfig);
    let isVerified = false;
    try {
        isVerified = yield etherscan.isVerified(address);
    }
    catch (err) {
        if (!force || err instanceof errors_1.NetworkRequestError) {
            throw err;
        }
...
`);
    ...
    const contractInformation = yield run(task_names_2.TASK_VERIFY_GET_CONTRACT_INFORMATION, {
        contractFQN,
        deployedBytecode,
        matchingCompilerVersions,
        libraries,
    });
    const minimalInput = yield run(task_names_2.TASK_VERIFY_ETHERSCAN_GET_MINIMAL_INPUT, {
        sourceName: contractInformation.sourceName,
    });
    const encodedConstructorArguments = yield (0, utilities_1.encodeArguments)(contractInformation.contractOutput.abi, contractInformation.sourceName, contractInformation.contractName, constructorArgs);
    // First, try to verify the contract using the minimal input
    const { success: minimalInputVerificationSuccess } = yield run(task_names_2.TASK_VERIFY_ETHERSCAN_ATTEMPT_VERIFICATION, {
        address,
...
`);
    ...
    const { success: fullCompilerInputVerificationSuccess, message: verificationMessage, } = yield run(task_names_2.TASK_VERIFY_ETHERSCAN_ATTEMPT_VERIFICATION, {
        address,
...
(0, config_1.subtask)(task_names_2.TASK_VERIFY_ETHERSCAN_RESOLVE_ARGUMENTS)
    ...
    .addOptionalParam("contract")
    .addFlag("force")
    .setAction(({ address, constructorArgsParams, constructorArgs: constructorArgsModule, contract, libraries: librariesModule, force, }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const constructorArgs = yield (0, utilities_1.resolveConstructorArguments)(constructorArgsParams, constructorArgsModule);
    let libraries;
...
(0, config_1.subtask)(task_names_2.TASK_VERIFY_ETHERSCAN_GET_MINIMAL_INPUT)
    .addParam("sourceName")
    .setAction(({ sourceName }, { run }) => __awaiter(void 0, void 0, void 0, function* () {
    const cloneDeep = require("lodash.clonedeep");
    const dependencyGraph = yield run(task_names_1.TASK_COMPILE_SOLIDITY_GET_DEPENDENCY_GRAPH, { sourceNames: [sourceName] });
    ...
    const compilationJob = yield run(task_names_1.TASK_COMPILE_SOLIDITY_GET_COMPILATION_JOB_FOR_FILE, {
        dependencyGraph,
        file: resolvedFiles[0],
    });
    const minimalInput = yield run(task_names_1.TASK_COMPILE_SOLIDITY_GET_COMPILER_INPUT, {
        compilationJob,
    });
...
(0, config_1.subtask)(task_names_2.TASK_VERIFY_ETHERSCAN_ATTEMPT_VERIFICATION)
    ...
    .setAction(({ address, compilerInput, contractInformation, verificationInterface, encodedConstructorArguments, }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const { message: guid } = yield verificationInterface.verify(address, JSON.stringify(compilerInput), contractFQN, `v${contractInformation.solcLongVersion}`, encodedConstructorArguments);
    console.log(`Successfully submitted source code for contract
${contractFQN} at ${address}
for verification on the block explorer. Waiting for verification result...
`);
    // Compilation is bound to take some time so there's no sense in requesting status immediately.
    yield (0, utilities_1.sleep)(700);
    const verificationStatus = yield verificationInterface.getVerificationStatus(guid);
