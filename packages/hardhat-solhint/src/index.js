...
function hasConfigFile(rootDirectory) {
...
function readIgnore(rootDirectory) {
    try {
        return fs
            .readFileSync((0, path_1.join)(rootDirectory, ".solhintignore"))
            .toString()
            .split("\n")
            .map((i) => i.trim())
            .filter(Boolean);
...
function getSolhintConfig(rootDirectory) {
    return __awaiter(this, void 0, void 0, function* () {
        let solhintConfig;
        const { loadConfig, applyExtends, } = require("solhint/lib/config/config-file");
        if (yield hasConfigFile(rootDirectory)) {
            try {
                solhintConfig = yield loadConfig();
            }
            catch (err) {
...
(0, config_1.subtask)("hardhat-solhint:run-solhint", (_, { config }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const solFilesGlob = (0, path_1.join)(config.paths.sources, "**", "*.sol");
    ...
    const relativeGlob = (0, path_1.relative)(config.paths.root, solFilesGlob);
    ...
    return processPath(normalizedGlob, yield getSolhintConfig(config.paths.root));
}));
(0, config_1.task)("check", (_, { run }, runSuper) => __awaiter(void 0, void 0, void 0, function* () {
    if (runSuper.isDefined) {
        yield runSuper();
    }
    const reports = yield run("hardhat-solhint:run-solhint");
    printReport(reports);
    const errorsCount = reports.reduce((acc, i) => {
