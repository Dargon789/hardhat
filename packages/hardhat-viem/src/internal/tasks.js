 ...
 * `hre.artifacts`.
 */
(0, config_1.subtask)(task_names_1.TASK_COMPILE_SOLIDITY).setAction((_, { config, artifacts }, runSuper) => __awaiter(void 0, void 0, void 0, function* () {
    const superRes = yield runSuper();
    const duplicateContractNames = yield findDuplicateContractNames(artifacts);
    const duplicateArtifactsDTs = generateDuplicateArtifactsDefinition(duplicateContractNames);
    try {
        yield (0, promises_1.writeFile)((0, path_1.join)(config.paths.artifacts, "artifacts.d.ts"), duplicateArtifactsDTs);
    }
    catch (error) {
 ...
 */
(0, config_1.subtask)(task_names_1.TASK_COMPILE_SOLIDITY_EMIT_ARTIFACTS).setAction((_, { artifacts, config }, runSuper) => __awaiter(void 0, void 0, void 0, function* () {
    const { artifactsEmittedPerFile } = yield runSuper();
    const duplicateContractNames = yield findDuplicateContractNames(artifacts);
    yield Promise.all(artifactsEmittedPerFile.map(({ file, artifactsEmitted }) => __awaiter(void 0, void 0, void 0, function* () {
        const srcDir = (0, path_1.join)(config.paths.artifacts, file.sourceName);
        yield (0, promises_1.mkdir)(srcDir, {
            recursive: true,
        });
        const contractTypeData = yield Promise.all(artifactsEmitted.map((contractName) => __awaiter(void 0, void 0, void 0, function* () {
            const fqn = (0, contract_names_1.getFullyQualifiedName)(file.sourceName, contractName);
            const artifact = yield artifacts.readArtifact(fqn);
 ...
 * a matching `.sol` file.
 */
(0, config_1.subtask)(task_names_1.TASK_COMPILE_REMOVE_OBSOLETE_ARTIFACTS).setAction((_, { config, artifacts }, runSuper) => __awaiter(void 0, void 0, void 0, function* () {
    const superRes = yield runSuper();
    const fqns = yield artifacts.getAllFullyQualifiedNames();
    const existingSourceNames = new Set(fqns.map((fqn) => (0, contract_names_1.parseFullyQualifiedName)(fqn).sourceName));
    const allArtifactsDTs = yield (0, fs_utils_1.getAllFilesMatching)(config.paths.artifacts, (f) => f.endsWith("artifacts.d.ts"));
    for (const artifactDTs of allArtifactsDTs) {
        ...
        if (!existingSourceNames.has(sourceName)) {
            yield (0, promises_1.rm)(dir, { force: true, recursive: true });
        }
    }
    return superRes;
}));
 ...
 */
function findDuplicateContractNames(artifacts) {
