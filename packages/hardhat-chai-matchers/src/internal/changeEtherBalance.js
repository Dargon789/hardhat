...
function getBalanceChange(transaction, account, options) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const block = yield provider.send("eth_getBlockByHash", [
            txReceipt.blockHash,
            false,
        ]);
        ...
        const balanceAfterHex = yield provider.send("eth_getBalance", [
            address,
            `0x${txBlockNumber.toString(16)}`,
        ]);
        const balanceBeforeHex = yield provider.send("eth_getBalance", [
            address,
            `0x${(txBlockNumber - 1).toString(16)}`,
        ]);
        const balanceAfter = BigInt(balanceAfterHex);
        const balanceBefore = BigInt(balanceBeforeHex);
        if ((options === null || options === void 0 ? void 0 : options.includeFee) !== true && address === txResponse.from) {
