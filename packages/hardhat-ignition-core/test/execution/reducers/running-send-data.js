...
describe("DeploymentStateReducer", () => {
    describe("running a named library deploy", () => {
        ...
        let updatedSendDataExState;
        const initializeSendDataExecutionStateMessage = {
        ...
        const revertedTransactionMessage = {
            type: messages_1.JournalMessageType.TRANSACTION_CONFIRM,
            futureId: "SendData1",
            networkInteractionId: 1,
            hash: "0xdeadbeef",
            receipt: {
                ...
                status: jsonrpc_1.TransactionReceiptStatus.FAILURE,
                logs: [],
            },
        };
        const sendDataSuccessMessage = {
        ...
        const sendDataFailsWithRevertMessage = {
            type: messages_1.JournalMessageType.SEND_DATA_EXECUTION_STATE_COMPLETE,
            futureId: "SendData1",
            result: {
        ...
        const sendDataFailOnStrategyError = {
            type: messages_1.JournalMessageType.SEND_DATA_EXECUTION_STATE_COMPLETE,
            futureId: "SendData1",
            result: {
        ...
        const sendDataFailOnSimulationError = {
            type: messages_1.JournalMessageType.SEND_DATA_EXECUTION_STATE_COMPLETE,
            futureId: "SendData1",
            result: {
        ...
        describe("initialization", () => {
            beforeEach(() => {
                ...
                updatedSendDataExState = (0, find_execution_state_by_id_1.findExecutionStateById)(execution_state_1.ExecutionStateType.SEND_DATA_EXECUTION_STATE, updatedDeploymentState, "SendData1");
            });
            it("should populate a call execution state for the future", () => {
                chai_1.assert.equal(updatedSendDataExState.type, execution_state_1.ExecutionStateType.SEND_DATA_EXECUTION_STATE);
            });
        });
        describe("strategy requesting an onchain interaction", () => {
            ...
            it("should populate a new onchain interaction", () => {
                chai_1.assert.equal(updatedSendDataExState.networkInteractions.length, 1);
                const networkInteraction = updatedSendDataExState.networkInteractions[0];
        ...
        describe("execution engine sends transaction", () => {
            ...
            it("should populate the transaction against the network interaction", () => {
                const networkInteraction = (0, find_onchain_interaction_by_1.findOnchainInteractionBy)(updatedSendDataExState, 1);
                chai_1.assert.equal(networkInteraction.transactions.length, 1);
                const transaction = (0, find_transaction_by_1.findTransactionBy)(updatedSendDataExState, 1, "0xdeadbeef");
        ...
        describe("transaction confirms but is an error", () => {
            ...
            it("should set the receipt against the successful transaction", () => {
                const transaction = (0, find_transaction_by_1.findTransactionBy)(updatedSendDataExState, 1, "0xdeadbeef");
                chai_1.assert.deepStrictEqual(transaction.receipt, revertedTransactionMessage.receipt);
            });
            it("should clear all other transactions for the network interaction", () => {
                const networkInteraction = (0, find_onchain_interaction_by_1.findOnchainInteractionBy)(updatedSendDataExState, 1);
                chai_1.assert.equal(networkInteraction.transactions.length, 1);
        ...
        describe("strategy indicates send data completes successfully", () => {
            ...
            it("should set the result against the execution state", () => {
                chai_1.assert.deepStrictEqual(updatedSendDataExState.result, {
                    type: execution_result_1.ExecutionResultType.SUCCESS,
                });
            });
            it("should update the status to success", () => {
                chai_1.assert.deepStrictEqual(updatedSendDataExState.status, execution_state_1.ExecutionStatus.SUCCESS);
            });
        });
        describe("send data errors on a revert", () => {
            ...
            it("should update the status to failed", () => {
                chai_1.assert.equal(updatedSendDataExState.status, execution_state_1.ExecutionStatus.FAILED);
            });
        });
