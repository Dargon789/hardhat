 ...
 *  network interaction.
 */
function monitorOnchainInteraction(exState, jsonRpcClient, transactionTrackingTimer, requiredConfirmations, millisecondBeforeBumpingFees, maxFeeBumps, getTransactionRetryConfig = {
    maxRetries: 10,
    retryInterval: 1000,
}, disableFeeBumping) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const transaction = yield _getTransactionWithRetry(jsonRpcClient, lastNetworkInteraction, getTransactionRetryConfig, exState.id);
...
function _getTransactionWithRetry(jsonRpcClient, onchainInteraction, retryConfig, futureId) {
    return __awaiter(this, void 0, void 0, function* () {
        let transaction;
        ...
        for (let i = 0; i < retryConfig.maxRetries; i++) {
            ...
            const transactions = yield Promise.all(onchainInteraction.transactions.map((tx) => jsonRpcClient.getTransaction(tx.hash)));
            ...
            yield new Promise((resolve) => setTimeout(resolve, retryConfig.retryInterval));
        }
        return transaction;
    });
}
