 ...
 */
function runStrategy(exState, executionStrategy) {
    return __awaiter(this, void 0, void 0, function* () {
        const strategyGenerator = yield (0, replay_strategy_1.replayStrategy)(exState, executionStrategy);
        const lastNetworkInteraction = exState.networkInteractions.at(-1);
        let response;
        if (lastNetworkInteraction === undefined) {
            response = yield strategyGenerator.next();
        }
        else if (lastNetworkInteraction.type === network_interaction_1.NetworkInteractionType.ONCHAIN_INTERACTION) {
            (0, assertions_1.assertIgnitionInvariant)(exState.type !== execution_state_1.ExecutionStateType.STATIC_CALL_EXECUTION_STATE, `Unexpected StaticCallExecutionState ${exState.id} with onchain interaction ${lastNetworkInteraction.id} when trying to run a strategy`);
            ...
            response = yield typedGenerator.next({
                type: execution_strategy_1.OnchainInteractionResponseType.SUCCESSFUL_TRANSACTION,
                transaction: Object.assign(Object.assign({}, confirmedTx), { receipt: Object.assign(Object.assign({}, confirmedTx.receipt), { status: jsonrpc_1.TransactionReceiptStatus.SUCCESS }) }),
            });
        }
        else {
            ...
            response = yield strategyGenerator.next(lastNetworkInteraction.result);
        }
...
function resolveNetworkInteractionRequest(exState, req) {
    ...
    if (req.type === network_interaction_1.NetworkInteractionType.STATIC_CALL) {
        return Object.assign(Object.assign({}, req), { from: (_a = req.from) !== null && _a !== void 0 ? _a : exState.from });
    }
    return req;
}
