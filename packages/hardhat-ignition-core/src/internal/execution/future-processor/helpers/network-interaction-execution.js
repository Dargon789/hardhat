 ...
 */
function runStaticCall(client, staticCall) {
 ...
 */
function sendTransactionForOnchainInteraction(client, sender, onchainInteraction, nonceManager, decodeSimulationResult, deploymentLoader, futureId) {
    var _a;
    return __awaiter(this, void 0, void 0, function* () {
        const nonce = (_a = onchainInteraction.nonce) !== null && _a !== void 0 ? _a : (yield nonceManager.getNextNonce(sender));
        const fees = yield getNextTransactionFees(client, onchainInteraction);
        ...
        let gasLimit;
        try {
            gasLimit = yield client.estimateGas(estimateGasPrams);
        }
        catch (error) {
            ...
            const { fees: _fees } = estimateGasPrams, paramsWithoutFees = __rest(estimateGasPrams, ["fees"]);
            // If the gas estimation failed, we simulate the transaction to get information
            // about why it failed.
            const failedEstimateGasSimulationResult = yield client.call(paramsWithoutFees, "pending");
            const decoded = yield decodeSimulationResult(failedEstimateGasSimulationResult);
        ...
        const transactionParams = Object.assign(Object.assign({}, estimateGasPrams), { gasLimit });
        ...
        // simulation fails.
        const simulationResult = yield client.call(transactionParams, "pending");
        const decodedSimulationResult = yield decodeSimulationResult(simulationResult);
        if (decodedSimulationResult !== undefined) {
            return decodedSimulationResult;
        }
        yield deploymentLoader.recordToJournal({
            type: messages_1.JournalMessageType.TRANSACTION_PREPARE_SEND,
            futureId,
            networkInteractionId: onchainInteraction.id,
            nonce: transactionParams.nonce,
        });
        const txHash = yield client.sendTransaction(transactionParams);
        return {
 ...
 */
function getNextTransactionFees(client, onchainInteraction) {
    return __awaiter(this, void 0, void 0, function* () {
        const recommendedFees = yield client.getNetworkFees();
