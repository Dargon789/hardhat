...
const get_network_execution_states_1 = require("../views/execution-state/get-network-execution-states");
...
class ExecutionEngine {
     ...
     */
    executeModule(deploymentState, module, batches, accounts, deploymentParameters, defaultSender) {
        return __awaiter(this, void 0, void 0, function* () {
            yield this._checkForMissingTransactions(deploymentState);
            deploymentState = yield this._syncNonces(deploymentState, module, accounts, defaultSender);
            yield this._executionStrategy.init(this._deploymentLoader, this._jsonRpcClient);
            ...
            for (const batch of batches) {
                ...
                deploymentState = yield this._executeBatch(futureProcessor, executionBatch, deploymentState);
     ...
     * @returns The new deployment state.
     */
    _executeBatch(futureProcessor, batch, deploymentState) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let block = yield this._jsonRpcClient.getLatestBlock();
            while (true) {
                for (const future of sortedFutures) {
                    const { newState } = yield futureProcessor.processFuture(future, deploymentState);
                    deploymentState = newState;
                }
                if ((0, is_batch_finished_1.isBatchFinished)(deploymentState, sortedFutures.map((f) => f.id))) {
                    break;
                }
                block = yield this._waitForNextBlock(block);
            }
            return deploymentState;
        });
    }
     ...
     */
    _waitForNextBlock(previousBlock) {
        return __awaiter(this, void 0, void 0, function* () {
            while (true) {
                yield new Promise((resolve) => setTimeout(resolve, this._blockPollingInterval));
                const newBlock = yield this._jsonRpcClient.getLatestBlock();
     ...
     */
    _checkForMissingTransactions(deploymentState) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const exState of exStates) {
                for (const ni of exState.networkInteractions) {
                    if (ni.type === network_interaction_1.NetworkInteractionType.ONCHAIN_INTERACTION &&
                        ni.nonce !== undefined &&
                        ni.transactions.length === 0) {
                        throw new errors_1.IgnitionError(errors_list_1.ERRORS.EXECUTION.TRANSACTION_LOST, {
                            futureId: exState.id,
                            nonce: ni.nonce,
                            sender: exState.from,
                        });
     ...
     */
    _syncNonces(deploymentState, ignitionModule, accounts, defaultSender) {
        return __awaiter(this, void 0, void 0, function* () {
            const nonceSyncMessages = yield (0, get_nonce_sync_messages_1.getNonceSyncMessages)(this._jsonRpcClient, deploymentState, ignitionModule, accounts, defaultSender, this._requiredConfirmations);
            for (const message of nonceSyncMessages) {
                deploymentState = yield (0, deployment_state_helpers_1.applyNewMessage)(message, deploymentState, this._deploymentLoader);
            }
            return deploymentState;
        });
    }
