 ...
 */
function getNonceSyncMessages(jsonRpcClient, deploymentState, ignitionModule, accounts, defaultSender, requiredConfirmations) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const block = yield jsonRpcClient.getLatestBlock();
        ...
        for (const [sender, pendingIgnitionTransactions] of Object.entries(pendingTransactionsPerSender)) {
            ...
            const safeConfirmationsCount = confirmedBlockNumber !== undefined
                ? yield jsonRpcClient.getTransactionCount(sender, confirmedBlockNumber)
                : undefined;
            const pendingCount = yield jsonRpcClient.getTransactionCount(sender, "pending");
            const latestCount = yield jsonRpcClient.getTransactionCount(sender, "latest");
            ...
            for (const { nonce, transactions, executionStateId, networkInteractionId, } of pendingIgnitionTransactions) {
                const fetchedTransactions = yield Promise.all(transactions.map((tx) => jsonRpcClient.getTransaction(tx)));
...
function createMapFromSenderToNonceAndTransactions(deploymentState, ignitionModule, accounts, defaultSender) {
    ...
    for (const executionState of Object.values(deploymentState.executionStates)) {
        if (executionState.type ===
            ...
            executionState.type === execution_state_1.ExecutionStateType.CONTRACT_AT_EXECUTION_STATE ||
            executionState.type ===
                execution_state_1.ExecutionStateType.ENCODE_FUNCTION_CALL_EXECUTION_STATE) {
            continue;
        }
