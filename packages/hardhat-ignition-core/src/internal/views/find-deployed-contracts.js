...
function findDeployedContracts(deploymentState) {
    return Object.values(deploymentState.executionStates)
        .filter((exState) => exState.type === execution_state_1.ExecutionStateType.DEPLOYMENT_EXECUTION_STATE ||
...
function _toDeployedContract(des) {
    switch (des.type) {
        ...
        case execution_state_1.ExecutionStateType.CONTRACT_AT_EXECUTION_STATE: {
            return {
                id: des.id,
                contractName: des.contractName,
